...
export async function isCharacter(selected){
    await fetch(window.location+"/"+selected).then(response=>response.json()).then(result=>{
        ...
        let building=`<div  class="foundPerk bg-dark rounded d-flex gap-4" >`


        if(result['gender']==="YES"){
            building+=`<div  class="foundPerk foundPerk bg-green rounded ">${result["selected_killer"]["gender"]}</div>`
        }else if(result['gender']==="MAYBE"){
            building+=`<div  class="foundPerk foundPerk bg-yellow rounded ">${result["selected_killer"]["gender"]}</div>`
        }
        else{
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["gender"]}</div>`
        }if(result['origin']==="YES"){
            building+=`<div  class="foundPerk foundPerk rounded  bg-green">${result["selected_killer"]["origin"]}</div>`
        }else if(result['origin']==="MAYBE"){
            building+=`<div  class="foundPerk foundPerk rounded  bg-yellow">${result["selected_killer"]["origin"]}</div>`
        }
        else{
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["origin"]}</div>`
        }if(result['height']==="YES"){
            building+=`<div  class="foundPerk foundPerk rounded bg-green">${result["selected_killer"]["height"]}</div>`
        }else{
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["height"]}</div>`
        }if(result['movement_speed']==="YES"){
            building+=`<div  class="foundPerk foundPerk rounded bg-green">${result["selected_killer"]["movement_speed"]}</div>`
        }else{
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["movement_speed"]}</div>`
        }if(result['power_attack_type']==="YES"){
            building+=`<div  class="foundPerk foundPerk rounded bg-green">${result["selected_killer"]["power_attack_type"]}</div>`
        }else{
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["power_attack_type"]}</div>`
        }if(result['year']==="YES"){
            building+=`<div  class="foundPerk foundPerk rounded bg-green">${result["selected_killer"]["year"]}</div>`
        }else if(result['year']==="HIGHER"){
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["year"]} ⬆️</div>`
        }else if(result['year']==="LOWER"){
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["year"]} ⬇️</div>`
        }else{
            building+=`<div  class="foundPerk alreadyGuessed rounded ">${result["selected_killer"]["year"]}</div>`
        }
        building+=`</div>`
