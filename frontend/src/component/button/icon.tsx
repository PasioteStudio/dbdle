...
const IconButton: React.FC<IconButton> = ({link,icon}) => {
    ...
    <li className=" rounded-full bg-red-900 cursor-pointer grid" onClick={()=>{router.push(link)}}>
        <Image src={icon} alt="perk guessing icon" width={40} height={40} className='p-1 grid-column-1' />
        {visible && <Image src={"/imgs/logos/check_mark.webp"} alt="check mark" width={30} height={30} className='p-1 ml-auto -mr-3 -mt-3 relative grid-column-1' />}
    </li>
    );
};
