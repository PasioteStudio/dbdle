    ...
    <div>
      ...
      <div className="grid text-center -mt-20">
        ...
        <div className="grid-column-1 mt-25">
          ...
          <div className="p-5 w-[80%] mx-auto aspect-square justify-items-center grid bg-no-repeat bg-contain bg-center">
            <h1 className="text-3xl mt-10 ">{quote ? "❝"+quote + "❞" : "Loading..."}</h1>
          </div>
          {missCount < 6 ? 
          <div className='grid h-28 w-full -mt-54'>
            <Image src={"/imgs/effects/ui_cloud.png"} alt="" width={500} height={500} className="grid-column-1 w-full overflow-visible -z-10 -mt-64" />
            <div className={`grid-column-1 h-20 w-[40%] mx-auto ${missCount > 0 ? "" : "cursor-pointer"}`} onClick={handleHintClick}>
              <Image src={"/imgs/logos/random_perk.png"} className="w-15 mx-auto" alt="hint icon" width={160} height={160} />
              <p>Perk Clue {missCount > 0 ? `after ${missCount} tries` : ""}</p>
            </div>
          </div> : null}
          <div className={`mx-auto ${missCount < 6 ? "mt-4" : "-mt-40"}`}>
